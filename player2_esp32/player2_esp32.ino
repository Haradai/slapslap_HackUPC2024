...
void callback(char *topic, byte *payload, unsigned int length) {
    ...
    if (strcmp(topic, "/Turn") == 0 && strcmp(lastMessage_tourn, "Player1") == 0) {
        turn = 1;
        analogWrite(RED, 0);
        analogWrite(GREEN, 0);
        analogWrite(YELLOW, 255);
    } else if (strcmp(topic, "/Turn") == 0 && strcmp(lastMessage_tourn, "Player2") == 0) {
        turn = 2;
        analogWrite(RED, 0);
        analogWrite(GREEN, 255);
        analogWrite(YELLOW, 0);
    }
    if (strcmp(topic, "/Game_state") == 0 && strcmp(lastMessage_game, "Hit success") == 0 && turn == 1 ){
      for(int i=0; i<10;i++){
        analogWrite(RED, 255);
        analogWrite(GREEN, 0);
        analogWrite(YELLOW, 0);
